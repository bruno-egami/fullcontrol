...
# ==============================================================================
# 1. CONFIGURAÇÕES DO USUÁRIO (PARÂMETROS GEOMÉTRICOS E DE PROCESSO)
# ==============================================================================

# --- Entrada de Arquivo Vetorial ---
VETOR_ARQUIVO = "no-celta.dxf"       # Nome/caminho do arquivo vetorial de entrada (.svg ou .dxf)
MODO_LINHA_UNICA = True             # Se True, imprime apenas a linha em si (Vase Mode de Linha Única), sem gerar infill ou perímetros na base sólida.

# --- Parâmetros Geométricos Tridimensionais ---
largura_desejada_x = 100.0           # Largura física real que a peça deve ter no eixo X na primeira camada (mm)
x_centro, y_centro = 180.0, 45.0    # Coordenadas do centro geométrico desejado da peça na mesa (mm)
...
zonas_camadas = [
    {
        'camada_inicio': 0,
        'num_perimetros': 0,        # 0 = apenas infill (com ampliação automática de 1 perímetro extra)
    ...
    {
        'camada_inicio': 4,         # Inicia o vaso em vase mode a partir da camada 3
        'num_perimetros': 1,
        'infill_percent': 0.0,
        'infill_pattern': 'concentric',
...
if MODO_LINHA_UNICA:
    NUM_CAMADAS_BASE_MACICA = 2     # Número de camadas de base maciça sólida (fechada) sob o nó celta
...
gcode = fc.transform(steps, 'gcode', fc.GcodeControls(
    ...
    save_as='vetor_inclinado_solido',
    initialization_data={
        'primer': 'no_primer', 
        'dia_feed': 1.75,
...
arquivos = sorted(glob.glob('vetor_inclinado_solido*.gcode'), key=os.path.getmtime)
if arquivos:
    ...
    print(f"G-code vetorial gerado com sucesso! -> {arquivo_gcode}")
